...
/* ===================== GO HOME ===================== */
function goHome() {
  // Fade the right panel out, then navigate
...
function goToStep(n) {
  ...
  setTimeout(() => {
    ...
    nxt.querySelectorAll('.anim-up').forEach((el, i) => {
      ...
      el.offsetHeight;
      el.style.animation = `slideUp .55s var(--ease) both`;
...
document.getElementById('photoInput').addEventListener('change', function(e) {
  ...
  reader.onload = function(ev) {
    ...
    ring.style.transform = 'scale(1.1) rotate(-3deg)';
    setTimeout(() => {
      ring.style.transform = 'scale(1.04) rotate(0deg)';
...
photoRing.addEventListener('dragover', e => {
  e.preventDefault();
  photoRing.style.transform = 'scale(1.07)';
});
photoRing.addEventListener('dragleave', () => {
  ...
  if (el) {
    el.addEventListener('input', () => {
      ...
      const map = {
        ...
        cgpa: 'f-cgpa',
        percent10: 'f-percent10',
        percent12: 'f-percent12',
        school12Name: 'f-school12',
      };
      validateField(map[id] || 'f-' + id.toLowerCase(), el);
    });
  }
});
...
document.getElementById('email').addEventListener('input', function() {
  ...
  document.querySelector('#f-email .field-hint').textContent =
...
document.querySelectorAll('.course-card').forEach(card => {
  card.addEventListener('click', function() {
    ...
    document.querySelectorAll('.tag').forEach(t => {
      if (selectedCourse.toLowerCase().includes(
          t.textContent.toLowerCase().split(' ')[0].toLowerCase())) {
